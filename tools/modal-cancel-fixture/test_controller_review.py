...
class ModalControllerReviewTests(unittest.TestCase):
    def _native_case(self, root, *, owner_error=False, cleanup_ok=True, require_live_post=False, supervisor_error=False):
        ...
        state = {'hwnd': 222, 'main_title': 'main', 'dialog_title': 'dialog', 'dialog_body': 'body', 'dialog_closed': True}
        checks = {'state': state, 'counters': {'cancel': 1, 'yes': 0}}
        payload = {'status': 'PASS', 'direct_cancel_invoke_count': 1}
        supervisor_report = {'status': 'SUCCESS', 'stdout': 'diagnostic sentinel', 'cleanup_verified': True}
        result = SimpleNamespace(success=True, child_payload=payload, cleanup_verified=True, to_dict=lambda: supervisor_report)
        ...
        with mock.patch.object(controller, 'os', SimpleNamespace(name='nt')), mock.patch.object(
            ...
            report = controller.run_native(root.parent / 'output')
